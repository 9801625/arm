...
constexpr int motor_id_gm = 2;//main : 2 sabu : 1
...
constexpr int motor_id_3 = 3;
constexpr uint32_t mode_switch_id_3 = 0x700 + motor_id_3;
...
/*下記に参考資料
 // --- setPWM()を使ってサーボを動かす（パルス幅のステップ指定） ---
  Serial.println(servonum);  // 現在のサーボ番号を表示

  for (uint16_t pulselen = SERVOMIN; pulselen < SERVOMAX; pulselen++) {
    pwm.setPWM(servonum, 0, pulselen);  // サーボを少しずつ動かす
  }
  delay(500);  // 少し待つ

  for (uint16_t pulselen = SERVOMAX; pulselen > SERVOMIN; pulselen--) {
    pwm.setPWM(servonum, 0, pulselen);  // サーボを逆方向へ動かす
  }
  delay(500);

  // --- writeMicroseconds()を使って動かす（マイクロ秒指定） ---
